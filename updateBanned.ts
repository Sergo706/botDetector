...
export async function updateBannedIP(
  ...
  const params = [ cookie, ipAddress, country, user_agent, reasonPayload ].map(v => v === undefined ? null : v);
  
  try { 
  await pool.execute(
    `INSERT INTO banned (canary_id, ip_address, country, user_agent, reason)
     VALUES (?, ?, ?, ?, ?)`,
    params
  );
