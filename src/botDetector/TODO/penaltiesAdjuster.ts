...
interface BannedRow {
    ...
    avg_total_ban_score: number;
    percent_contribution: number;
    contribution_severity: string;
}
...
export async function adjustPenalties(cookie: string): Promise<void> {
    ...
    `;
    const [bans] = await pool.execute<RowDataPacket[]>(totalBanned);
    const totalBans = Number(bans[0]['COUNT(canary_id)']);
    const [analysis] = await pool.execute<RowDataPacket[]>(query);
    ...
    if (reasonsToSkip) return;

   const percentageForEachReason: number = parseFloat(((total_occurrences / totalBans) * 100).toFixed(2));
   ...
   const reasonToSettingMap: Record<string, string> = {
