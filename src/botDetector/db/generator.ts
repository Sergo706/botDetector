...
async function buildBannedMmdb(generateTypes: boolean, mmdbctlPath: string): Promise<void> {
    const TEMP_DIR = path.resolve(getLibraryRoot(), '_data-sources', `.tmp_banned_${Date.now().toString()}`);
    ...
    try {
        ...
        log.error('Failed to build banned.mmdb', err);
        throw err;
    } finally {
...
async function buildHighRiskMmdb(scoreThreshold: number, generateTypes: boolean, mmdbctlPath: string): Promise<void> {
    ...
    const TEMP_DIR = path.resolve(MMDB_DIR, `.tmp_highrisk_${Date.now().toString()}`);
    const db = getDb();
    ...
    try {
        ...
        log.error('Failed to build highRisk.mmdb', err);
        throw err;
    } finally {
        await fs.rm(TEMP_DIR, { recursive: true, force: true });
    }
