...
import { urlListOfGoogleIPs, bingIPList, openAiIPList, appleIPList, ahrefsIPList, duckDuckGoIPList, commonCrawlerIPList, xAndTwitterIPList, facebookIPList, pinterestIPList, telegramIPList, semrushIPList, } from '../db/json/urls.js';
...
async function fetchProvider(name, urls) {
    const raws = await Promise.all(urls.map(u => fetch(u).then(res => {
        if (!res.ok)
            throw new Error(`fetch ${name} ${u} → ${res.status}`);
        return res.json();
    })));
...
async function fetchIPsFromDocs(urls, regex) {
    const texts = await Promise.all(urls.map(fetchPageWithCurl));
    const allMatches = texts.flatMap(txt => [...txt.matchAll(new RegExp(regex, 'g'))].map(m => m[0]));
    return Array.from(new Set(allMatches));
}
async function appendSnapshot(filePath, provider, data) {
    let history = [];
    try {
