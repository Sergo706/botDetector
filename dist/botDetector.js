...
export async function uaAndGeoBotDetector(req, ipAddress, userAgent, geo, parsedUA) {
    ...
    if (settings.checks.enableIpChecks) {
        cheapChecks.push(async function ipCheck() {
            ...
            return {
                ...
                reasons: isValid ? [] : ['INVALID_IP']
            };
        });
    }
    if (settings.checks.enableGoodBotsChecks) {
        cheapChecks.push(async function goodBotCheck() {
            const { score, isBadBot, isGoodBot } = await validateGoodBots(data.browserType, data.browser, ipAddress);
            if (isBadBot)
                throw new BadBotDetected();
            if (isGoodBot)
                throw new GoodBotDetected();
            return { score, reasons: [] };
        });
    }
    if (settings.checks.enableBrowserAndDeviceChecks) {
        cheapChecks.push(async function browserDetailsAndDeviceCheck() {
            return calculateBrowserDetailsAndDevice(data.browserType, data.browser, data.os, data.device, data.deviceVendor, data.browserVersion, data.deviceModel);
        });
    }
    ...
    if (settings.checks.enableProxyIspCookiesChecks) {
        checks.push(async function proxyIspCookieCheck() {
            return calculateProxyIspAndCookie(cookie, data.proxy, data.hosting, data.isp, data.org, data.as);
        });
    }
    ...
    if (settings.checks.enableGeoChecks) {
        checks.push(async function geoLocationCheck() {
            return calculateGeoLocation(data.country, data.region, data.regionName, data.lat, data.lon, data.district, data.city, data.timezone);
        });
    }
    try {
